function CGame(oData) {
  ...
  this.generateFinalSymbols = function () {
    ...
    for (var k = 0; k < _iLastLineActive; k++) {
      ...
      for (var t = iStartIndex; t < aCombos.length; t++) {
        if (
          _aFinalSymbolCombo[aCombos[t].row][aCombos[t].col] === iValue ||
  ...
  this._endReelAnimation = function () {
    ...
    const spinData = {
      spinPrice: _iTotBet,
      outcome: _aWinningLine.length > 0,
      moneyEarned: _iTotWin,
    };

    $(s_oMain).trigger("save_spin", spinData);
  };
